...
namespace caffe {
	...
	void RPNLayer<Dtype>::LayerSetUp(
		...
		generate_anchors(); //zs生成多尺度和多比例的锚点
	...
	template <typename Dtype>
	void RPNLayer<Dtype>::generate_anchors(){//生成9中后选区域
	...
	void RPNLayer<Dtype>::Forward_cpu(
		...
		nms(aboxes,nms_thresh_); //zs非极大值抑制
		//end = clock();
		//std::cout << "sort nms:" << (double)(end - start) / CLOCKS_PER_SEC << std::endl;
		if (post_nms_topN_ > 0) //zs当label>0，也就是有object时，将会进行box的回归
		{
